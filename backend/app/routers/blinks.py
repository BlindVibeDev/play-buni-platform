...
from app.services.solana_actions import (
    solana_actions_service,
    SolanaAction,
    ActionType,
    BlinkStatus,
)
...
class CreateBlinkRequest(BaseModel):
    ...
    action_type: str = Field(
        default="swap", description="Type of action (swap, buy, sell)"
    )
    input_token: str = Field(default="USDC", description="Input token symbol")
    suggested_amounts: Optional[List[float]] = Field(
        default=None, description="Suggested amounts for quick actions"
    )
    max_slippage: float = Field(
        default=1.0, description="Maximum slippage percentage", ge=0.1, le=10.0
    )
...
class ExecuteActionRequest(BaseModel):
    ...
    slippage: Optional[float] = Field(
        default=0.5, description="Slippage tolerance", ge=0.1, le=10.0
    )
    priority_fee: Optional[int] = Field(
        default=0, description="Priority fee in lamports"
    )
...
class BlinkCreateResponse(BaseModel):
    """Simple response when creating a Blink."""
...
@router.post("/create", response_model=BlinkCreateResponse)
async def create_blink(
    ...
    try:
        ...
        result = await db.execute(select(Signal).where(Signal.id == request.signal_id))
        ...
        async with solana_actions_service as service:
            blink_id = await service.generate_and_store_blink(
            ...
            return BlinkCreateResponse(id=blink_id, message="Blink generated")
...
async def get_blink_action(blink_id: str):
    ...
    try:
        async with get_db() as db:
            ...
            result = await db.execute(select(Blink).where(Blink.blink_id == blink_id))
...
async def execute_blink_action(
    blink_id: str, request: ExecuteActionRequest, db: AsyncSession = Depends(get_db)
):
    """
        ...
        for trade in trades:
            trade_responses.append(
                TradeResponse(
                    ...
                    output_amount=(
                        float(trade.output_amount) if trade.output_amount else None
                    ),
...
async def get_treasury_balance(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve treasury balance"
        )
    ...
    try:
        ...
        blinks_result = await db.execute(
            select(
                ...
                func.count(Blink.id)
                .filter(Blink.status == BlinkStatus.ACTIVE.value)
                .label("active_blinks"),
                func.count(Blink.id)
                .filter(Blink.status == BlinkStatus.EXECUTED.value)
                .label("executed_blinks"),
        ...
        if blinks_stats.total_blinks > 0:
            execution_rate = (
                blinks_stats.executed_blinks or 0
            ) / blinks_stats.total_blinks
        ...
        return {
            ...
            "trading": {
                ...
                "total_volume": (
                    float(trades_stats.total_volume)
                    if trades_stats.total_volume
                    else 0.0
                ),
                "total_fees": (
                    float(trades_stats.total_fees) if trades_stats.total_fees else 0.0
                ),
...
async def handle_transaction_update(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to process transaction update"
        )
