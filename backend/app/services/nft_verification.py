...
class NFTVerificationService:
    ...
    def _parse_metadata_account(self, account_data: bytes) -> Optional[NFTMetadata]:
        ...
        try:
            ...
            seller_fee_basis_points = int.from_bytes(data[offset:offset+2], 'little')
            offset += 2
            ...
            if has_creators:
                ...
                for _ in range(creator_count):
                    creator_address = base58.b58encode(data[offset:offset+32]).decode()
                    offset += 32
            ...
            return NFTMetadata(
                ...
                token_standard=None,  # Would need additional parsing
                collection=None,      # Would need additional parsing
                uses=None            # Would need additional parsing
            )
        ...
        try:
            ...
            async with self.session.get(uri, timeout=10) as response:
                if response.status == 200:
                    ...
                    logger.warning(f"Failed to fetch metadata from {uri}: {response.status}")
                    return {}
    ...
    async def verify_nft_ownership(
        ...
        try:
            ...
            # Verify ownership
            ownership = await self._get_token_account_by_owner(owner_pubkey, mint_pubkey)
    ...
    async def verify_collection_membership(
        ...
        try:
            ...
            return (
                metadata.collection.get("key") == collection_address and
                metadata.collection.get("verified", False)
            )
