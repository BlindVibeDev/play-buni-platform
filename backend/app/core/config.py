...
class Settings(BaseSettings):
    ...
    required_nft_collection: Optional[str] = Field(default=None, env="REQUIRED_NFT_COLLECTION")
    ...
    jupiter_free_endpoint: str = Field(default="https://www.jupiterapi.com", env="JUPITER_FREE_ENDPOINT")
    quicknode_metis_endpoint: Optional[str] = Field(default=None, env="QUICKNODE_METIS_ENDPOINT")
    jupiter_paid_endpoint: Optional[str] = Field(default=None, env="JUPITER_PAID_ENDPOINT")
    jupiter_api_url: str = Field(default="https://quote-api.jup.ag/v6", env="JUPITER_API_URL")
    jupiter_swap_api_url: str = Field(default="https://quote-api.jup.ag/v6/swap", env="JUPITER_SWAP_API_URL")
    ...
    premium_rate_limit_per_minute: int = Field(default=1000, env="PREMIUM_RATE_LIMIT_PER_MINUTE")
    enable_rate_limiting: bool = Field(default=True, env="ENABLE_RATE_LIMITING")
    
    # Signal Configuration
    signal_processing_interval: int = Field(default=30, env="SIGNAL_PROCESSING_INTERVAL")
    ...
    market_data_update_interval: int = Field(default=30, env="MARKET_DATA_UPDATE_INTERVAL")  # seconds
    websocket_heartbeat_interval: int = Field(default=120, env="WEBSOCKET_HEARTBEAT_INTERVAL")  # seconds
    connection_cleanup_interval: int = Field(default=300, env="CONNECTION_CLEANUP_INTERVAL")  # seconds
    ...
    platform_fee_bps: int = Field(default=100, env="PLATFORM_FEE_BPS")  # 1% fee (100 basis points)
    min_trade_amount: float = Field(default=1.0, env="MIN_TRADE_AMOUNT")  # Minimum trade amount in USD
    max_trade_amount: float = Field(default=100000.0, env="MAX_TRADE_AMOUNT")  # Maximum trade amount in USD
    ...
    def admin_wallets_list(self) -> List[str]:
        ...
        if not self.admin_wallets:
            return []
        return [wallet.strip() for wallet in self.admin_wallets.split(",") if wallet.strip()]
